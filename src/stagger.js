...
class Stagger extends Tunable {
  ...
  */
  _getOptionByMod(name, i, store) {
    ...
    let value = h.isArray(props) ? props[i % props.length] : props;
  ...
  */
  _init(options, Module) {
    let count = this._getChildQuantity(options.quantifier || "el", options);
    this._createTimeline(options);
    this._modules = [];
    for (let i = 0; i < count; i++) {
      // get child module's option
      let option = this._getOptionByIndex(i, options);
