...
import resize from 'vendor/resize';
...
class MotionPath {
  ...
  // ### Class body docs
  // ---
  constructor(o={}){ this.calcHeight = this.calcHeight.bind(this);   this.o = o; if (this.vars()) { return; } this.createTween(); this; }

  vars() {
    this.getScaler = h.bind(this.getScaler, this); this.resize = resize;
  ...
  curveToPath(o){
    ...
    const endPoint  = {x: start.x + o.shift.x, y: start.x + o.shift.y};
    const {
      curvature
    } = o;

    const dX = o.shift.x; const dY = o.shift.y;
    const radius = Math.sqrt((dX*dX) + (dY*dY)); const percent = radius/100;
    let rotation  = (Math.atan(dY/dX)*(180/Math.PI)) + 90;
    if (o.shift.x < 0) { rotation = rotation + 180; }
...
${endPoint.x},${endPoint.y}`
  ...
  postVars() {
    ...
    this.rotate = 0; this.speedX = 0; this.speedY = 0; this.blurX = 0; this.blurY = 0;
    this.prevCoords = {}; this.blurAmount = 20;
...
DOMNode or another module.`
  ...
  addEvent(el, type, handler){ return el.addEventListener(type, handler, false); }
  removeEvent(el, type, handler){ return el.removeEventListener(type, handler, false); }
  createFilter() {
  ...
  <filter id="${this.filterID}" y="-20" x="-20" width="40" height="40">
    ...
    const svg = div.querySelector(`#svg-${this.filterID}`);
  ...
  parseEl(el){
    ...
    if (el._setProp != null) { this.isModule = true; return el; }
  }

  getPath() {
    const path = h.parsePath(this.props.path); if (path) { return path; }
  ...
  getScaler(){
    ...
    const size = {}; this.scaler = {};
    size.width  = end.x >= start.x ? end.x-start.x : start.x-end.x;
    size.height = end.y >= start.y ? end.y-start.y : start.y-end.y;

    switch (this.fillRule) {
      case 'all':
        this.calcWidth(size); return this.calcHeight(size);
      case 'width':
        this.calcWidth(size);  return this.scaler.y = this.scaler.x;
      case 'height':
        this.calcHeight(size); return this.scaler.x = this.scaler.y;
    }
  }
      // else @calcBoth(size)
...
be overridden on run yet`
  ...
  createTween() {
    this.tween = new Tween({
      ...
      onStart:    () => (this.props.onStart != null ? this.props.onStart.apply(this) : undefined),
      onComplete: () => {
        this.props.motionBlur && this.setBlur({
          blur: {x: 0, y: 0}, offset: {x: 0, y: 0}});
        return (this.props.onComplete != null ? this.props.onComplete.apply(this) : undefined);
      },
      onUpdate:  p=> this.setProgress(p),
      onFirstUpdate:(isForward, isYoyo)=> {
        if (!isForward) { return (this.history.length > 1) && this.tuneOptions(this.history[0]); }
      }});
  ...
  startTween() { return setTimeout((() => (this.timeline != null ? this.timeline.play() : undefined)), 1); }
  ...
  setProgress(p, isInit){
    ...
    const x = point.x + this.props.offsetX; const y = point.y + this.props.offsetY;
  ...
  _getCurrentRotation(point, len, p){
    ...
    if (this.props.isRotation || (this.props.rotationOffset != null) || isTransformFunOrigin) {
      ...
      const x1 = point.y - prevPoint.y; const x2 = point.x - prevPoint.x;
      let atan = Math.atan(x1/x2); !isFinite(atan) && (atan = 0);
      this.rotate = atan*h.RAD_TO_DEG;
      if ((typeof this.props.rotationOffset) !== 'function') {
        return this.rotate += this.props.rotationOffset || 0;
      } else { return this.rotate = this.props.rotationOffset.call(this, this.rotate, p); }
    } else { return this.rotate = 0; }
  }
  _setTransform(x,y,p,isInit){
    // get real coordinates relative to container size
    if (this.scaler) { x *= this.scaler.x; y *= this.scaler.y; }
    // call onUpdate but not on the very first(0 progress) call
    let transform = null;
    if (!isInit) { transform = typeof this.onUpdate === 'function' ? this.onUpdate(p, { x, y, rotate: this.rotate }) : undefined; }
    // set position and rotatation
    // 1: if motion path is for module
    if (this.isModule) { return this.setModulePosition(x,y);
    // 2: if motion path is for DOM node
    } else {
      // if string was returned from the onUpdate call
      // then set this string to the @el
      if (typeof transform !== 'string') { return this.setElPosition(x,y,p);
      } else { return h.setPrefixedStyle(this.el, 'transform', transform); }
    }
  }
  ...
  makeMotionBlur(x, y){
    ...
    let tailRotation = 0; let signX = 1; let signY = 1;
    if ((this.prevCoords.x == null) || (this.prevCoords.y == null)) { this.speedX = 0; this.speedY = 0;
    // else calculate speed based on the largest axes delta
    } else {
      const dX = x-this.prevCoords.x; const dY = y-this.prevCoords.y;
      if (dX > 0) { signX = -1; }
      if (signX < 0) { signY = -1; }
      this.speedX = Math.abs(dX); this.speedY = Math.abs(dY);
    ...
    this.prevCoords.x = x; return this.prevCoords.y = y;
  }
  ...
  then(o){
    const prevOptions = this.history[this.history.length-1]; const opts = {};
    for (let key in prevOptions) {
      ...
      } else { if (o[key] == null) { o[key] = undefined; } }
    ...
    this.history.push(o); const it = this;
    opts.onUpdate      = p=> this.setProgress(p);
    opts.onStart       = () => (this.props.onStart != null ? this.props.onStart.apply(this) : undefined);
    opts.onComplete    = () => (this.props.onComplete != null ? this.props.onComplete.apply(this) : undefined);
    opts.onFirstUpdate = function() { return it.tuneOptions(it.history[this.index]); };
    opts.isChained = !o.delay;
    this.timeline.append(new Tween(opts));
    return this;
  }

  tuneOptions(o){ this.extendOptions(o); return this.postVars(); }

  rotToCoords(rotation){
    rotation = rotation % 360;
    const radRotation = ((rotation-90)*Math.PI)/180;
    let x = Math.cos(radRotation); let y = Math.sin(radRotation);
