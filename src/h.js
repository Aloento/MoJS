...
padding: 1px 5px 2px; border: 1px solid #FF512F;`;
  ...
  vars() {
    ...
    this.getRemBase();
    this.isFF = this.prefix.lowercase === 'moz'; this.isIE = this.prefix.lowercase === 'ms';
  ...
  cloneObj(obj, exclude){
    const keys = Object.keys(obj); const newObj = {}; let i = keys.length;
    while(i--) {
      const key = keys[i];
      if (exclude != null) { if (!exclude[key]) { newObj[key] = obj[key]; }
  ...
  extend(objTo, objFrom){
    for (let key in objFrom) {
      ...
      if (objTo[key] == null) { objTo[key] = objFrom[key]; }
    }
    return objTo;
  }
  ...
  style(el, name, value){
    if (typeof name === 'object') {
      const keys = Object.keys(name); let len = keys.length;
      return (() => {
        const result = [];
        while(len--) {
          const key = keys[len]; value = name[key];
          result.push(this.setPrefixedStyle(el, key, value));
        }
        return result;
      })();
    } else { return this.setPrefixedStyle(el, name, value); }
  }

  prepareForLog(args){
    args = Array.prototype.slice.apply(args);
    args.unshift('::'); args.unshift(this.logBadgeCss); args.unshift('%cmo·js%c');
    return args;
  }
  log() {
    if (mojs.isDebug === false) { return; }
  ...
  parseUnit(value){
    ...
    if (typeof value === 'number') {
      ...
      let unit = __guard__(value.match(regex), x => x[0]); let isStrict = true;
      // if a plain number was passed set isStrict to false and add px
      if (!unit) { unit = 'px'; isStrict = false; }
  ...
  getPrefix() {
    ...
    const pre = (v || ((styles.OLink === "") && [
      "",
      "o"
    ]))[1];
    const dom = __guard__(("WebKit|Moz|MS|O").match(new RegExp("(" + pre + ")", "i")), x => x[1]);
    return {
      dom,
      lowercase: pre,
      css: "-" + pre + "-",
      js: (pre != null ? pre[0].toUpperCase() : undefined) + (pre != null ? pre.substr(1) : undefined)
    };
  }
  strToArr(string){
    const arr = [];
  ...
  isArray(variable){ return variable instanceof Array; }

  normDashArrays(arr1, arr2){
    // if !arr1? or !arr2? then throw Error 'Two arrays should be passed'
    let currItem, i, lenDiff, startI;
    const arr1Len = arr1.length; const arr2Len = arr2.length;
    if (arr1Len > arr2Len) {
      let asc, end;
  ...
  makeColorObj(color){
    ...
    if (color[0] !== '#') {
      ...
      if (!isRgb) {
        ...
        } else { return this.shortColors[color];
      }
        })();
      }
  ...
  computedStyle(el){ return getComputedStyle(el); }
  ...
  parseRand(string){
    ...
    if (units.unit && randArr[2].match(units.unit)) {return rand + units.unit;
    } else { return rand; }
  }
  parseStagger(string, index){
    ...
    value = (() => {
      ...
      base = this.parseUnit(this.parseIfRand(splittedValue[1])); return splittedValue[3];
    // if just a plain value
    } else { base = this.parseUnit(0); return splittedValue[1];
  }
    })();
    ...
    const unit = base.isStrict ? base.unit
    : unitValue.isStrict ? unitValue.unit : '';

    if (unit) { return `${number}${unit}`; } else { return number; }
  }
  ...
  parseIfStagger(value, i){
    if (!((typeof value === 'string') && value.match(/stagger/g))) { return value;
    } else { return this.parseStagger(value, i); }
  }


  // if passed string has rand function then get the rand value
  parseIfRand(str){
    if ((typeof str === 'string') && str.match(/rand\(/)) { return this.parseRand(str);
    } else { return str; }
  }
  ...
  parseDelta(key, value, index){
    ...
    if (easing != null) { easing = mojs.easing.parseEasing( easing ); }
    delete value.easing;
    // parse delta curve
    let {
      curve
    } = value;
    if (curve != null) { curve = mojs.easing.parseEasing( curve ); }
    delete value.curve;
    ...
    if (isNaN(parseFloat(start)) && !start.match(/rand\(/) && !start.match(/stagger\(/)) {
      if (key === 'strokeLinecap') {
        this.warn(`Sorry, stroke-linecap property is not animatable \
...
property, mo · js will fallback to end \"${end.unit}\" unit `
  ...
  delta(start, end){
    const type1 = typeof start; const type2 = typeof end;
...
but got - ${start}, ${end}`
    ...
    const obj = {}; obj[start] = end; return obj;
  }
  // ---
  ...
  parsePath(path){
    if (typeof path === 'string') {
      if (path.charAt(0).toLowerCase() === 'm') {
        ...
        domPath.setAttributeNS(null, 'd', path); return domPath;
      } else { return document.querySelector(path); }
    }
    if (path.style) { return path; }
  }
  // ---
  ...
  // @method parsePath
  // @return {SVGPath}
  closeEnough(num1, num2, eps){ return Math.abs(num1-num2) < eps; }
  // ---
  ...
  checkIf3d() {
    ...
    const {
      style
    } = div; const prefixed = `${this.prefix.css}transform`;
  ...
  */
  isObject(variable){ return (variable !== null) && (typeof variable === 'object'); }
  /*
  ...
  */
  getDeltaEnd(obj){ const key = Object.keys(obj)[0]; return obj[key]; }
  /*
  ...
  */
  getDeltaStart(obj){ const key = Object.keys(obj)[0]; return key; }
  /*
  ...
  */
  isTweenProp(keyName){ return this.tweenOptionMap[keyName] || this.callbacksMap[keyName]; }
  /*
  ...
  */
  getLastItem(arr) { return arr[arr.length-1]; }
  /*
