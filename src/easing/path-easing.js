...
class PathEasing {
  ...
  _vars() {
    ...
    this._step = 1/this._precompute; this._rect = this.o.rect || 100;
  ...
  constructor(path, o={}){
    ...
    this.o = o;
    if (path === 'creator') { return; }
    this.path = h.parsePath(path);
    if ((this.path == null)) { return h.error('Error while parsing the path'); }
  ...
  _findBounds(array, p){
    ...
    if (p === this._boundsPrevProgress) { return this._prevBounds; }
    ...
    if (this._boundsPrevProgress > p) { loopEnd = 0; direction = 'reverse';
    } else { loopEnd = len; direction = 'forward'; }

    // set default start and end bounds to the
    // very first and the very last items in array
    if (direction === 'forward') { start = array[0]; end = array[array.length-1];
    ...
    for (let i = this._boundsStartIndex, end1 = loopEnd, asc = this._boundsStartIndex <= end1; asc ? i < end1 : i > end1; asc ? i++ : i--) {
      const value = array[i]; let pointX = value.point.x/this._rect; let pointP = p;
      // if direction is reverse swap pointX and pointP
      // for if statement
      if (direction === 'reverse') {
        const buffer = pointX; pointX = pointP; pointP = buffer;
      }
      ...
      if (pointX < pointP) {
        start = value; this._boundsStartIndex = i;
  ...
  sample(p){
    ...
    const res = this._checkIfBoundsCloseEnough(p, bounds); if (res != null) { return res; }
  ...
  _checkIfPointCloseEnough(p, point){
    if (h.closeEnough(p, point.x/this._rect, this._eps)) { return this._resolveY(point); }
  }
  // ---
  ...
  _findApproximate(p, start, end, approximateMax = this._approximateMax){
    ...
    const point = this.path.getPointAtLength(approximation); const x = point.x/this._rect;
    // if close enough resolve the y value
    if (h.closeEnough(p, x, this._eps)) { return this._resolveY(point);
    } else {
      // if looping for a long time
      if (--approximateMax < 1) { return this._resolveY(point); }
  ...
  _resolveY(point){ return 1 - (point.y/this._rect); }
  // ---
  //
  ...
  _normalizePath(path){
    ...
    points.shift(); const commands = path.match(svgCommandsRegexp);
  ...
  _normalizeSegment(segment, value=0){
    ...
    const x = lastPoint[0]; const parsedX = Number(x);
