 ...
 */
import h from 'src/h';
import bezier from 'easing/bezier-easing';
import PathEasing from 'easing/path-easing';
import mix from 'easing/mix';
import { default as approximate } from 'easing/approximate';
...
class Easing {
  static initClass() {
    ...
    // EASINGS
    this.prototype.linear =   {none(k) { return k; }};
    this.prototype.ease = {
    ...
    this.prototype.sin = {
      in(k) { return 1 - Math.cos((k * PI) / 2); },
      out(k) { return sin((k * PI) / 2); },
      inout(k) { return 0.5 * (1 - Math.cos(PI * k)); }
    };
    this.prototype.quad = {
      in(k) { return k * k; },
      out(k) { return k * (2 - k); },
    ...
    this.prototype.expo = {
      in(k) { if (k === 0) { return 0; } else { return Math.pow(1024, k - 1); } },
      out(k) { if (k === 1) { return 1; } else { return 1 - Math.pow(2, -10 * k); } },
      inout(k) {
        if (k === 0) { return 0; }
        if (k === 1) { return 1; }
  ...
  parseEasing(easing){
    ...
    if ((easing == null)) { easing = 'linear.none'; }
